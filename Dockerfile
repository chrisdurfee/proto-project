...
# Copy modules directory structure for autoloader
COPY modules/ ./modules/

# Create directory structure for bind mount and copy key files
RUN mkdir -p /var/www/html/public /var/www/html/common/Migrations \
    && echo "<?php // Placeholder for bind mount ?>" > /var/www/html/preload.php

# Copy sync-config.js if it exists (needed for configuration)
COPY sync-config.js* ./

# Generate autoloader with infrastructure files
